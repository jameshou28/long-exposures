...
- Current phase: **Phase 5** (frame registration) done in code. Phases 0–4 (the v1 ship point: import → select → blend → export + in-app library) done in code. Everything builds clean and launches in the simulator (landing + library toolbar button verified via screenshot). The **editor, blend, export, and registration paths are unverified** because the simulator has no Live Photos — needs a real-device run.
...
- Phase 5: registration aligns around the **centre of the current selection**, not the whole clip — so the part of the scene being blended is what stays sharp. `RegistrationService` (Vision) estimates per-frame transforms to a reference index (homography → translation → identity fallback) and `apply(_:to:using:)` warps a buffer via Core Image (`CIPerspectiveTransformWithExtent` for homography, affine for translation). `FrameStore` holds only the raw frames; `previewFrames(for:)` / `fullResolutionFrames(for:)` return the aligned slice for a range when `isRegistered`, with the reference = `lower + referenceIndex(count)`. Transforms are cached per reference index (`transformCache`), so dragging the *far* handle (centre unchanged) reuses the Vision result; moving the centre recomputes. Off-actor work goes through `RegistrationService.alignedSlice`/`transformsOffActor` with a `PixelBufferBox: @unchecked Sendable` wrapper (CoreVideo `CVBuffer` isn't Sendable). `EditorViewModel`: when registration is on, the blend/export bypass the engine's range cache (aligned pixels vary with the selection) and call the uncached `engine.blend(frames:mode:)`; when off, the cached `blend(frames:range:mode:)` path is kept for instant drag-replay. `EditorView` has an "Align frames" `Toggle` with a spinner (`isRegistering`). Thumbnails always show raw frames (the strip is a scrubber).
