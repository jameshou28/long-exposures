...
- Current phase: **Phase 4** (export + in-app library) — the **v1 ship point**. Code complete, builds clean and launches in the simulator (landing + library toolbar button verified via screenshot). The **editor, blend, and export paths are unverified** because the simulator has no Live Photos — needs a real-device run. Phases 0–3 done.
