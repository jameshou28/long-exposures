...
- Phase 8 (so far): **onboarding**. `OnboardingView` = 3-card paged `TabView` (`.page` style) — "Make a long exposure" / "You pick the frames" / "Choose how they blend" — with Skip + Next/Get Started. Gated on `AppSettings.hasSeenOnboarding` (UserDefaults bool): `ContentView.onAppear` shows it as a `fullScreenCover` on first launch, sets the flag on finish/skip. Re-openable via a "How it works" row in `SettingsView` (passes an `onShowOnboarding` closure that dismisses settings + presents onboarding). **Visually verified** in the sim on a fresh install (first card renders correctly with icon, copy, page dots, Skip, Next).
- Phase 8 (so far): **ship prep**. **App icon** — `Assets.xcassets/AppIcon.appiconset` has a 1024×1024 blue-aperture-on-dark icon (light/dark slots use `icon_1024.png`, opaque RGB no-alpha so it passes validation; tinted uses grayscale `icon_1024_tinted.png`). Generated by `scratchpad/make_icon.py` (Pillow, six-blade iris). **`SHIP.md`** (repo root) has the full ship kit: App Store listing copy (name/subtitle/description/keywords), the "No data collected" privacy stance + a hostable privacy-policy text, a 5–6 screenshot shot-list, the TestFlight/submission checklist, and a pre-submit on-device test pass. The $99 enrollment + actual submission are manual (marked [you]).
- **Still TODO in Phase 8**: per-frame include/exclude, reference-frame picker, empty-state polish — all optional; the app is feature-complete and ship-ready pending the on-device pass + the manual App Store steps in SHIP.md.
